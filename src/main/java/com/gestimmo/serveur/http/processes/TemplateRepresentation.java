...
public class TemplateRepresentation extends OutputRepresentation {
	...
	public static TemplateRepresentation createNew(final String template,
			final Context context) {
		return createNew(template, context, MediaType.TEXT_HTML);
	}

	public static TemplateRepresentation createNew(final String template,
			final Context context, final MediaType type) {
		return new TemplateRepresentation(template, context, type);
	}

	public TemplateRepresentation(final String template, final Context context,
			final MediaType type) {
		super(type);
		this.context = context;
		final Map<String, Object> data = Maps.newHashMap();
		representation = new org.restlet.ext.freemarker.TemplateRepresentation(
				template, getConfiguration(), data, type);
	}
	...
	private Configuration getConfiguration() {
		return (Configuration) context.getAttributes().get(
				"org.freemarker.Configuration");
	}
}
